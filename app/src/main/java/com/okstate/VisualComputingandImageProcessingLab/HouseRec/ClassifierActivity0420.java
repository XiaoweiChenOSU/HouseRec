...
public  class ClassifierActivity0420 extends CameraActivityBackupRT implements OnImageAvailableListener {
    ...
    runInBackground(
        new Runnable() {
          ...
          public void run() {
            ...
            multipartBodyBuilder.addFormDataPart("image" + time, "_train.jpg", RequestBody.create(MediaType.parse("image/*jpg"), byteData));
...
//            }
            ...
            if(ActivityCompat.shouldShowRequestPermissionRationale(ClassifierActivity0420.this,Manifest.permission.READ_EXTERNAL_STORAGE)){
                ActivityCompat.requestPermissions(ClassifierActivity0420.this,
...
//              LOGGER.v("Detect: %s", resultsClassifer);
              ...
              runOnUiThread(
                  new Runnable() {
                    @Override
                    public void run() {
//                      showResultsInBottomSheet(Results[0],Results[1]);
                    }
                  });
            }
            readyForNextImage();
  ...
  protected void onInferenceConfigurationChanged() {
    ...
    runInBackground(new Runnable() {
      @Override
      public void run() {
        ClassifierActivity0420.this.recreateClassifier(model, device, numThreads,isMymodel,layoutName);
      }
    });
  }
    ...
    if (device == Device.GPU
      ...
      runOnUiThread(
              new Runnable() {
                @Override
                public void run() {
                  Toast.makeText(ClassifierActivity0420.this, R.string.tfe_ic_gpu_quant_error, Toast.LENGTH_LONG).show();
                }
              });
      return;
    }
    try {
      ...
      runOnUiThread(
              new Runnable() {
                @Override
                public void run() {
                  Toast.makeText(ClassifierActivity0420.this, e.getMessage(), Toast.LENGTH_LONG).show();
                }
              });
      return;
    }
